...
```bash
# Git을 사용하는 경우 (dictionary 브랜치만 가져오기)
git clone -b dictionary https://github.com/2026TUKCOMCD/EchoMind.git
cd EchoMind
```
...
3. **규칙 1 (내 컴퓨터용)**: 
    ...
    - **소스**: **'내 IP (My IP)'** 선택 (현재 집 IP 자동 입력됨).
4. **규칙 2 (EC2 서버용)**: **'규칙 추가'** 버튼을 *한 번 더* 누르세요! (한 줄에 다 적는 게 아닙니다)
    - **유형**: `MYSQL/Aurora` (3306).
    - **소스**: **'사용자 지정'** 선택 -> 옆 빈칸에 EC2 보안 그룹 ID(예: `sg-014...`)를 검색해서 **클릭**.
5. **저장**. (총 2개의 규칙이 있어야 합니다)
